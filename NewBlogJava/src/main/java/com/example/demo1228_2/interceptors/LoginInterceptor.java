...
public class LoginInterceptor implements HandlerInterceptor { //拦截器
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        //更新user最后登录时间
        updatLast_visited_time(request);
     ...
     * @param request 1
     */
    private void updatLast_visited_time(HttpServletRequest request){
        User user = userMapper.selectById(Tool.getUserSessionId(request.getSession()));
        // 未登录直接返回
        if(user==null)return;
        user.setLast_visited_time(LocalDateTime.now());
        // 设置最后登录时间
