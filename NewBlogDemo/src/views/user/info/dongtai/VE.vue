<template>
  <div v-loading="loading">
    <div class="card" v-for="(obj,key) in list" :key="key">
        ...
        <div class="card-info" >
           ...
           <div style="color: #777;font-size: 0.8rem;margin-top: 4px;">
            ...
            <!-- 回复评论的内容 -->
            <div v-if="obj.type==='comment'" style="margin-top: 8px;">
            ...
            <!-- 引用评论 -->
            <div @click="click(obj)" v-if="obj.type!=='guanzhu' && obj.type!=='product_zan'" style="margin-top: 8px;cursor: pointer;" title="跳转到">
                <div style="border-left: 3px solid gainsboro;padding: 0 8px;"  >
                    <span @click.stop="$router.push('/user/otheruserinfo?user_id='+obj.user.id)" style="margin: 0 3px;color:cornflowerblue;cursor: pointer;">
    ...
    <p v-if="!loading && currentPage>=pages" style="color: #777;">没有更多了</p>
  </div>
</template>
    ...
    methods:{
        ...
        click(obj){
            const id = obj.product_id?
                obj.product_id:obj?.biao_obj?.product_id?
                    obj.biao_obj.product_id : obj?.obj_biao_receiver?.product_id?
                        obj.obj_biao_receiver.product_id:false
            if(!id){
                if(obj.type==="guanzhu")return
                return this.$fail("已被删除")
            }
            this.$router.push(`/product?id=${id}&bottom=true`)
        },
        getUnreadZanGuanzhuComment(){
            ...
            axios.get(`action/get/dongtai`,{ params })
                .then(response=>{
        ...
        func2(obj){
            if(obj.type==='zan'){
                return obj.biao_obj?obj.biao_obj.info:"评论已删除"
            }
            if(obj.type==='comment'){
                return obj.obj_biao_receiver?obj.obj_biao_receiver.info:"评论已删除"
            }
            if(obj.type==='product_comment'){
                return obj.obj_biao_user?obj.obj_biao_user.info:"评论已删除"
            }
