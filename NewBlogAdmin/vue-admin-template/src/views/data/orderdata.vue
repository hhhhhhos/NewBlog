<template>
  <div class="app-container">
    <div style="margin: 0 0 20px 0;">
      <el-button type="primary" plain @click="OpenDialog('删除订单')">删除</el-button>
      <el-button type="primary" plain @click="OpenDialog('查询订单')">查询</el-button>
      <el-tag
      ...
      <el-table-column
          ...
          <template slot-scope="scope" >
            ...
            <div v-else-if="column.label==='用户名'">
              {{tableData[scope.$index][column.prop].wechat_nickname?tableData[scope.$index][column.prop].wechat_nickname: tableData[scope.$index][column.prop].name}}
            </div>
            <div v-else-if="column.label==='头像'">
              <img 
              v-if="tableData[scope.$index][column.prop].wechat_headimgurl"
              loading="lazy" style="border-radius: 40px;width:40px;height: 40px;"
              :src="getimg(tableData[scope.$index][column.prop])">
            </div>
            <div v-else-if="column.label==='状态'">
              <el-tag v-if="tableData[scope.$index][column.prop]==='未支付'">{{ tableData[scope.$index][column.prop]}}</el-tag>
              <el-tag type="success" v-else>{{ tableData[scope.$index][column.prop]}}</el-tag>
            </div>
            ...
            <div v-else-if="column.label==='收货地'">
              {{ tableData[scope.$index][column.prop].info.join(",") 
              + exist(tableData[scope.$index][column.prop].detail)
              + exist(tableData[scope.$index][column.prop].name)
              + exist(tableData[scope.$index][column.prop].phone)}}
            </div>
            <div v-else>
    ...
    <el-dialog :title="this.dialog_title3" :visible.sync="dialogVisible3" width="60%" >
      ...
      <el-form  style="margin:0 20px 0 0;" label-position="right" ref="form2" :model="dialogdata3" label-width="80px">
        ...
        <el-form-item v-for="(column, index) in columns2" :key="index" :label="column.label" :prop="column.prop">
          ...
          <div style="display: flex;" v-else-if="column.label==='支付状态'">
            <el-radio-group v-model="dialogdata3[column.prop]" style="margin-left: 10px;">
  ...
  export default {
    ...
    data() {
      return {
        ...
        { prop: 'user_info', label: '用户名', width: '80' },
        { prop: 'user_info', label: '头像', width: '80' ,align:'center'},
        { prop: 'totalMoney', label: '总价', width: '80' },
        { prop: 'totalNum', label: '商品数', width: '80' },
        { prop: 'status', label: '状态', width: '100',align:'center' },
        { prop: 'info', label: '详细信息' },
        { prop: 'create_time', label: '创建时间', width: '180' },
        { prop: 'address', label: '收货地' , width: '280' },
        ],
        ...
        columns2 : [
          ...
          { prop: 'status', label: '支付状态', width: '80' },
    ...
    methods:{
      getimg(obj){
        console.log(obj)
        if(!obj)return null;
        else return (obj.wechat_headimgurl);
      },
      // 文字是否存在，存在返回
      exist(str){
        if(str!==null)
      ...
      handleImageError(event) {
        ...
        console.log(this.staticPath + "deletedproduct.webp")
        // 处理图片加载失败的逻辑
        event.target.src = this.staticPath + "deletedproduct.webp"
      },
      ...
      OpenDialog(val){
        ...
        if(val==="新增商品"){
          ...
          this.$confirm('确定删除这'+(this.rows_selection.length)+'个订单吗?', '提示', {
      ...
      // 删一堆
      delete_byadmin(){
        axios.post('/order/deletelistbyadmin',this.rows_selection)
        .then(response=>{
          if(response.data.code===0)this.$message.error(response.data.msg)
          else {
            this.dialogVisible2 = false
      ...
      gettablebycondition(){
        ...
        axios.get('/order/selectpagebyadmin',{
          params: params
        }).then(response=>{
          if(response.data.code===0)this.$message.error(response.data.msg)
          else {
